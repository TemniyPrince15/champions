...
function fallback(profile: Profile, mode: string, message = '') {
  ...
  return { ...base, summary:`Your profile is strongest around ${profile.major}. I would focus first on the highest-fit programs that also make sense for your budget and funding goal.`, actions:base.actions };
}
...
export async function POST(req: Request) {
  try {
    ...
    if(!process.env.OPENROUTER_API_KEY) return NextResponse.json(fallback(profile, mode, message));

    const catalog = programs.map(p=>({id:p.id,university:p.university,country:p.country,program:p.program,description:p.description,tuition:p.tuition.value,gpa:p.gpa.value,ielts:p.ielts.value,sat:p.sat.value,scholarship:p.scholarship.value,deadline:p.deadline.value}));
    const system = `You are Pathly AI, an admissions planning assistant. Analyze only the supplied student profile and university records. Never invent requirements, deadlines, scholarships, photos, or facts. Recommendations must use only supplied program IDs. Be concise, practical and explain trade-offs. If a fact is marked demo, tell the user to verify it on the official university site. Return JSON only with keys summary (string), actions (array of strings), recommendations (array of objects with id, university, program, reason).`;
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${process.env.OPENROUTER_API_KEY}`,'HTTP-Referer':process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000','X-Title':'Pathly AI'},body:JSON.stringify({model,messages:[{role:'system',content:system},{role:'user',content:user}],temperature:0.2,max_tokens:900})});
    if(!response.ok) return NextResponse.json(fallback(profile, mode, message));
    const data=await response.json();
    const text=typeof data?.choices?.[0]?.message?.content==='string'?data.choices[0].message.content:'';
    const parsed=parseJson(text);
    if(!parsed) return NextResponse.json(fallback(profile, mode, message));
    ...
    return NextResponse.json({summary:'Pathly could not complete the AI analysis right now. Your local recommendation engine is still available.',actions:[],recommendations:[],local:true},{status:200});
  }
}
